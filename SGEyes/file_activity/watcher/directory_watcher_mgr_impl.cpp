...
void directory_watcher_mgr_impl::checking_folder_move(watching_group& group)
{
	// get model
...
void directory_watcher_mgr_impl::checking_rename(watching_group& group)
{
	...
	int error{};
	bool isProcessing = fileIsProcessing(newName, error) || fileIsProcessing(oldName, error);
	if (isProcessing) {
...
void directory_watcher_mgr_impl::checking_create(watching_group& group)
{
	// get model
...
void directory_watcher_mgr_impl::checking_modify(watching_group& group)
{
	// get model
...
void directory_watcher_mgr_impl::checking_modify_without_modify_event(watching_group& group)
{
	// get model
	auto& model = group.mFileName.get_add();
...
void directory_watcher_mgr_impl::checking_copy(watching_group& group)
{
	// get model
	auto& model = group.mFileName.get_add();
...
void directory_watcher_mgr_impl::checking_move(watching_group& group)
{
	// get model
	auto& model = group.mFileName.get_add();
...
bool directory_watcher_mgr_impl::is_temporary_file(file_notify_info const& info, watching_group& group)
{
	// happen when download big file by save-as
	// will create -> remove -> waiting to rename
	auto key = info.get_path_wstring();
...
bool directory_watcher_mgr_impl::is_save_as_txt(file_notify_info const& info, watching_group& group)
{
	auto key = info.get_path_wstring();
	auto const& rmv = group.mFileName.get_remove().find(key);
	if (!rmv) {
...
bool directory_watcher_mgr_impl::is_create_only(file_notify_info const& info, watching_group& group)
{
	// **behaviour
	// step 1. create 1.txt
